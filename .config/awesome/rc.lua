...
-- This allows for floating window manager-like alt+tab
...
local function switch_keybinding(name)
    root.keys(keybindings[name])

    if name ~= "normal" then
        -- keybindingwidget is defined further down
        keybindingwidget.widget.text = name
    else
        keybindingwidget.widget.text = ""
    end
end

-- Returns a table which has reversed the previous table
...
beautiful.init(home .. "/.config/awesome/xresources/theme.lua")

-- This is defined after the theme definition so as to use the correct font
keybindingwidget = wibox.container.background(wibox.widget.textbox(), beautiful.get().bg_urgent)

-- This is used later as the default terminal and editor to run.
-- terminal = "xterm"
terminal = home .. "/dotfiles/multi/portable-term"
...
awful.layout.layouts = {
    ...
    -- These are deactivated by default
...
mytextclock = wibox.widget.textclock("%b %d %Y - %a, %I:%M %p")
...
awful.screen.connect_for_each_screen(function(s)
    ...
    s.mytasklist = awful.widget.tasklist {
        -- This reverses the order in which clients show up in the tasklist,
        -- which makes it resemble Vimium's tab interaction more closely
        -- source  = function () return reverse(awful.widget.tasklist.source.all_clients()) end,
        screen  = s,
...
-- {{{ Key bindings
keybindings["normal"] = gears.table.join(
    ...
    -- NOTE(Chris): I've actually not swapped these 'k' and 'j' bindings, since
    -- their default states are the opposite of how Vimium operates
    awful.key({ modkey,           }, "j",
        function ()
    ...
    awful.key({ modkey, "Shift" }, "r",
        function ()
            awful.spawn("killall cbatticon redshift") 
            awesome.restart()
        end,
              {description = "reload awesome", group = "awesome"}),
    ...
    awful.key({ modkey, "Control", "Shift" }, "d",
                function() awful.util.spawn(home .. "/dotfiles/minimalist/rofi/i3_exit.sh") end,
              {description = "use rofi to exit (i3?)", group = "awesome"}),

    awful.key({ modkey }, "o",
                function() switch_keybinding("launch") end,
                {description = "enter launch keybinding mode", group = "awesome"}),

    awful.key({ modkey, "Control", "Shift" }, "p",
                function() awful.util.spawn(home .. "/dotfiles/minimalist/rofi/power_options.sh") end,
                {description = "use rofi to manage power options", group = "awesome"})
)
...
clientkeys = gears.table.join(
    ...
    awful.key({ modkey, "Shift"   }, "t",
        ...
        {description = "toggle taskbar", group = "client"})
)
...
keybindings["launch"] = gears.table.join(
    awful.key({ }, "f", function() awful.util.spawn("firefox"); switch_keybinding("normal") end,
    ...
    awful.key({ }, "d",
                function() awful.util.spawn(home .. "/dotfiles/util/discord.sh"); switch_keybinding("normal") end,
                {description = "launch/open/move to Discord", group = "launcher"}),

    awful.key({ "Shift" }, "d",
                function() awful.util.spawn("killall Discord"); switch_keybinding("normal") end,
                {description = "launch/open/move to Discord", group = "launcher"}),
    ...
    awful.key({ "Shift" }, "q", 
                function()
                    awful.util.spawn("alacritty --title qalc -e qalc -s 'fractions off'")
                    switch_keybinding("normal")
                end,
                {description = "launch qalc in fractional mode", group = "launcher"}),
    ...
    awful.key({ }, "Escape",
                ...
                {description = "return to normal (mode) keybindings", group = "awesome" })
)

-- Set keys
root.keys(keybindings["normal"])
-- }}}
...
awful.rules.rules = {
    ...
    -- Add titlebars to normal clients and dialogs
    {
        ...
        except_any = { class = { "kitty", "Nautilus" }, maximized = { true } },
