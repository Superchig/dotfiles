...
# Source any local (machine-specific) configuration files that may print something
if [ -d "$HOME/.config/zsh/early-local" ]; then
  for FILE in $HOME/.config/zsh/early-local/*(D); do
    source $FILE
  done
fi
...
alias et="e ~/quasi-Ventoy/TODO.txt"
